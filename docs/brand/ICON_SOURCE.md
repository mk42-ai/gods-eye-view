...
| Icons available / shipped | 2,112 SVGs in the package; **58 shipped** in `public/icons/` (one per inventoried role + the `layers` fallback + five transit-mode names reserved for DOM surfaces) | PACKAGE — `ls node_modules/lucide-static/icons | wc -l`; manifest `src/ui/icons/lucide-manifest.json` |
...
58 icons. Each `public/icons/<name>.svg` is the SVGO-optimised, normalised copy of `node_modules/lucide-static/icons/<name>.svg` (lucide-static 1.47.0). The "before" digest is `sha256` of the package file exactly as installed from the registry tarball (PACKAGE, read 2026-09-19T05:47:16Z); the "after" digest is the shipped file. Full per-site detail is §3.1; re-running `node scripts/sync-lucide-icons.mjs` reproduces both columns (`JSON` on stdout).
...
| ↻ U+21BB | `orbit` | `public/icons/orbit.svg` (438 B; source 494 B) | `f0dd37c4c23e462ba4ed283ff8ae8fd288548999f8a4ca9f2f02c0aa7fdc4a17` | `fb5035f23643db9e32197686b4f714484097a8a96d4e7be6122ee6e5da4e1063` | ORBIT indicator beside the LOCATION controls (src/ui/locationControls.js) |
...
| 138 | `src/layers/directions/index.js:140` | ⇄ | U+21C4 | Directions row chips | SWAP A/B chip label | text "SWAP" (chip labels are text) |
| 139 | `src/ui/locationControls.js:146` | ↻ | U+21BB | ORBIT indicator | spinning-arrow mark before "ORBIT" | orbit |
...
| `→ ↔ ⇒ ⇔ ↘ ∓ ≈ ≤ ≥ −` | Code comments, debug logs and route strings ("AUS → LAX") | Prose punctuation; none is rendered as an icon. The gate scans the arrows block U+2190–U+21FF in code with comments blanked and only `→` allowed, so an arrow drawn as a button glyph fails while these do not. |
